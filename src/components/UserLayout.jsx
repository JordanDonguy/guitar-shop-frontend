  ...
  return (
    <div className="fade-in mb-30 flex min-h-screen px-[10%] pt-[140px]">
      <div className="sticky top-[140px] flex h-[80vh] w-1/6 flex-col border-r-2 border-neutral-400 max-2xl:w-1/4">
        <NavLink
          to="/user/profile"
          className={({ isActive }) => `mb-10 text-2xl hover:text-neutral-500 ${isActive ? "text-teal-500" : ""}`}
        >
          → User Profile
        </NavLink>
        <NavLink to="/user/orders" className={({ isActive }) => `mb-10 text-2xl hover:text-neutral-500 ${isActive ? "text-teal-500" : ""}`}>
          → Orders
        </NavLink>
      </div>
      <Outlet />
